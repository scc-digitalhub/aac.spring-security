 ...
 * <a href="https://tools.ietf.org/html/rfc8414#section-3.1">Authorization
 * Server Metadata Request</a> based on provided issuer and method invoked.
 *
...
class JwtDecoderProviderConfigurationUtils {
    ...
    static Map<String, Object> getConfigurationForOidcIssuerLocation(String issuer) {
        URI uri = URI.create(issuer);
        return getConfiguration(issuer, oidc(uri), oidcRfc8414(uri));
    }

    static Map<String, Object> getConfigurationForOauthIssuerLocation(String issuer) {
        URI uri = URI.create(issuer);
        // also support oauth metadata at wrong location,
        // as per oidc provider spec instead of oauth metadata
        return getConfiguration(issuer, oauth(uri), oauthAsOidc(uri));
    }
    ...
    static void validateIssuer(Map<String, Object> configuration, String issuer) {
        ...
        if (!issuer.equals(metadataIssuer)) {
            throw new IllegalStateException(
                    "The Issuer \"" + metadataIssuer + "\" provided in the configuration did not "
    ...
    private static URI oauthAsOidc(URI issuer) {
        return UriComponentsBuilder.fromUri(issuer)
                .replacePath(issuer.getPath() + OAUTH_METADATA_PATH)
                .build(Collections.emptyMap());
    }
}
